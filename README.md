...
- 其它
  ...
  - [x] **豆瓣榜单自动追踪**：从豆瓣热门/高分榜单获取影视，自动搜索转存，支持定时调度和失效检测
...
在 WebUI 中点击侧边栏「豆瓣追踪」即可使用。功能包括：

- **榜单选择**：从豆瓣电影（热门/最新/高分/冷门佳片）、电视剧、综艺榜单中勾选需要的分类
- **自动转存**：使用系统已配置的三源搜索（网络公开/CloudSaver/PanSou）查找夸克网盘资源并自动转存
- **筛选过滤**：支持按最低评分、年份范围筛选，按评分/年份排序
- **手动搜索**：输入影片名称直接搜索资源并一键转存
- **历史管理**：查看/删除转存历史，自动去重避免重复转存
- **失效检测**：检测已失效的分享链接并自动搜索替代资源
- **定时调度**：支持 Cron 表达式配置定时转存和定时失效检测

> 豆瓣追踪模块的转存历史存储在 `config/douban.db`（SQLite），与主任务列表独立互不影响。
