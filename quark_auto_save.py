...
class Quark:
    ...
    def save_file(self, fid_list, fid_token_list, to_pdir_fid, pwd_id, stoken):
        ...
        payload = {
            "fid_list": files_to_save,
            "fid_token_list": tokens_to_save,
        ...
        return {
            "code": response.get("code", -1),  # 确保返回包含 'code'
    ...
    def do_save_check(self, shareurl, savepath):
        try:
            ...
            files_to_save = fid_list[:MAX_SAVE_FILES]
            tokens_to_save = fid_token_list[:MAX_SAVE_FILES]
            names_to_save = file_name_list[:MAX_SAVE_FILES]
            
            print(f"转存文件ID列表: {files_to_save}")
            print(f"转存文件Token列表: {tokens_to_save}")
            print(f"转存文件名称列表: {names_to_save}")

            # 保存文件
            save_file = self.save_file(
                files_to_save, tokens_to_save, to_pdir_fid, pwd_id, stoken
            )
            ...
            if save_file["code"] == 41017:
                ...
                del_list = [
                    ...
                    if (item["file_name"] in names_to_save)
                    and ((datetime.now().timestamp() - item["created_at"]) < 60)
                ]
                if del_list:
                    for fid in del_list:
                        self.del_file(fid, pwd_id, stoken)
                return "转存成功"
            else:
                return f"转存失败: {save_file['message']}"
        except Exception as e:
            return f"转存过程中出现错误: {str(e)}"
    ...
    def dir_check_and_save(self, task, pwd_id, stoken, pdir_fid="", subdir_path=""):
        ...
        if fid_list:
            ...
            if save_file_return["code"] == 0:
                task_id = save_file_return["data"].get("task_id", "")
        ...
        # 打印实际保存的文件名
        if save_name_list:
            print(f"实际保存的文件名: {', '.join(save_name_list)}")
...
def do_save(account, tasklist=[]):
    ...
    total_files_transferred = 0  # 计数器初始化

    # 执行任务
    for index, task in enumerate(tasklist):
        # 判断任务期限
        if check_date(task):
            if total_files_transferred >= 2:  # 检查是否已转存超过2个文件
